...
@pytest.mark.asyncio
async def test_changes(hub, ctx, resource_group, location, storage_account):
    sku = 'Standard_LRS'
    kind = 'StorageV2'
    expected = {
...
async def test_absent(hub, ctx, resource_group, storage_account):
    expected = {
        ...
        "comment": f"Storage account Vault {keyvault} has been deleted.",
        "name": keyvault,
        "result": True,
    }
    ret = await hub.states.azurerm.storage.account.absent(ctx, storage_account, resource_group)
    assert ret["changes"]["new"] == expected["changes"]["new"] &&
           ret["changes"]["old"]["name"] == expected["changes"]["old"]["name"] &&
           ret["result"] == expected["result"]
