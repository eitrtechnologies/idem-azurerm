...
async def assignments_list_for_resource_group(hub, resource_group, **kwargs):  # pylint: disable=invalid-name
    '''
    .. versionadded:: 1.0.0
...
async def assignments_list(hub, **kwargs):
    ...
    polconn = await hub.exec.utils.azurerm.get_client('policy', **kwargs)
    try:
        policy_assign = await hub.exec.utils.azurerm.paged_object_to_list(polconn.policy_assignments.list())
...
async def definition_create_or_update(hub, name, policy_rule, **kwargs):  # pylint: disable=invalid-name
    '''
    .. versionadded:: 1.0.0
...
async def definitions_list(hub, hide_builtin=False, **kwargs):
    ...
    polconn = await hub.exec.utils.azurerm.get_client('policy', **kwargs)
    try:
        policy_defs = await hub.exec.utils.azurerm.paged_object_to_list(polconn.policy_definitions.list())
