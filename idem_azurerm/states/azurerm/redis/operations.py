...
async def present(hub, ctx, name, resource_group, location, sku, redis_configuration=None, enable_non_ssl_port=False,
    ...
    ret['comment'] = 'Failed to create Redis cache {0}! ({1})'.format(name, cache.get('error'))
    if not ret['result']:
        ret['changes'] = {}
    return ret
