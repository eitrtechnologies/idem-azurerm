...
async def present(
    ...
    **kwargs,
    ...
    :param vault_uri (Optional): The URI of the vault for performing operations on keys and secrets.
    ...
    :param enable_soft_delete: (Optional) A boolean value that specifies whether the 'soft delete' functionality is
    ...
    :param soft_delete_retention: (Optional) The soft delete data retention period in days. It accepts values between
        7-90, inclusive. Default value is 90.

    :param enable_purge_protection: (Optional, only effective if soft deleted is enabled) A boolean value specifying
        whether protection against purge is enabled for this vault. Setting this property to True activates protection
        against purge for this vault and its content - only the Key Vault service may initiate a hard, irrecoverable
        deletion. Enabling this functionality is irreversible - that is, the property does not accept False as its
        value.

    :param enable_rbac_authorization: (Optional) A boolean value that controls how data actions are authorized. When
        set to True, the key vault will use Role Based Access Control (RBAC) for authorization of data actions, and the
        access policies specified in vault properties will be ignored (warning: this is a preview feature). When set as
