...
async def create(
    ...
    **kwargs,
    ...
    :param sku: (Required) A dictionary representing the SKU of the Redis cache to deploy. Required parameters include:
    ...
    :param redis_configuration: (Optional) A dictionary of string key-value pairs that represent all Redis Settings.
    ...
    :param enable_non_ssl_port: (Optional) Specifies whether the non-ssl Redis server port (6379) is enabled.
        Defaults to False.

    :param tenant_settings: (Optional) A dictionary of tenant settings.

    :param shard_count: (Optional) The number of shards to be created on a Premium Cluster Cache.

    :param minimum_tls_version: (Optional) The specified TLS version (or higher) that clients are required to use.
        Possible values include: '1.0', '1.1', and '1.2'.

    :param subnet_id: (Optional) The full resource ID of a subnet in a virtual network to deploy the Redis cache in.
    ...
    :param static_ip: (Optional) Static IP address. Required when deploying a Redis cache inside an existing Azure
        Virtual Network.

    :param zones: (Optional) A list of availability zones denoting where the resource needs to come from.

    :param tags: (Optional) A dictionary of strings can be passed as tag metadata to the Redis cache object.

    :param polling: (Optional) A boolean flag representing whether a Poller will be used during the creation of the
